...
public function add(Request $req, ManagerRegistry $manager, UserRepository $repou,MailSender $mailSender, Security $security): Response
{
    $cours = new Cours();
    $form = $this->createForm(CoursType::class, $cours);
    $em = $manager->getManager();
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        $teacher= $repou->findOneBy(['email' => $security->getUser()->getUserIdentifier()]);
        ...
        $em->flush();
   //envoi mail
   $recipent = 'maram.hlali@esprit.tn';
   $subject = 'pdf file';
   $content ='Voici le PDF de votre cours.';
   

        $mailSender->sendEmail($recipent,$subject,$content,$fileName);
