...
function addParticipant(button,eventId,userId,eventName) {
    ...
    .then(data => {
        ...
        newDiv.innerHTML = `
            ...
            <div id="event_${ eventId }" class=" d-flex justify-content-start align-items-center flex-row gap-3">${eventName} 
        ...
        chatContainer.innerHTML+=` <div id="chat_${eventId}" class="row">
        <div id="chat${eventId}" class="contqiner" ></div>
        <input id="send${eventId}"  type="text" >
        <button type="button" class="sendBtn" data-send-id="${eventId}">${eventId}</button>
      </div>`
    }
        else{
